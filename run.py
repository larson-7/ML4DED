...
from ml4ded.run_utils.layer_detection import (
    group_layers_by_height,
    convert_layers_to_ids,
)
from ml4ded.run_utils.visualization import create_visualization, get_output_filename
from ml4ded.run_utils.metrics import (
    compute_temporal_metrics,
    analyze_layer_metrics,
    detect_anomalies,
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run model on single image, video, or directory of images."
    )

    input_group = parser.add_mutually_exclusive_group(required=True)
    input_group.add_argument(
        "--image", type=str, default="", help="Path to a single input image"
    )
    input_group.add_argument(
        "--video", type=str, default="", help="Path to a video file"
    )
    input_group.add_argument(
        "--image-dir", type=str, default="", help="Directory of input images"
    )
    parser.add_argument(
        "--model-weights-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        required=True,
        help="Expected real-world height (mm) of the final part (CURRENT_PART)",
    )
    parser.add_argument(
        "--enable-temporal", action="store_true", help="Enable temporal consistency"
    )
    parser.add_argument(
        "--frame-stride",
        default=10,
        type=int,
        help="number of frames to skip for each processed frame",
    )
    parser.add_argument("--device", default="cuda", help="Training device")
    parser.add_argument(
        "--color-layers", action="store_true", help="Color coordinate layers in plot"
    )
    parser.add_argument(
        "--use-median",
        action="store_true",
        help="Apply median filtering to height measurements",
    )
    parser.add_argument(
        "--filter-window",
        default=7,
        type=int,
        help="Window size for median filter (odd number)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-metrics", type=str, help="Path to save metrics JSON file"
    )
...
def download_weights_if_needed(
    model_weights_dir, use_temporal, repo_id="iknocodes/ml4ded"
):
...
def load_input_frames(args):
    ...
    if args.video:
        from ml4ded.data_processing.preparation.video_cropping import (
            crop_and_save_video,
        )
        ...
        _, frames_to_process = crop_and_save_video(
            input_video=video_path, output_dir=tmp_img_dir, stride=frame_stride
        )
...
def main():
    ...
    download_weights_if_needed(
        args.model_weights_dir, use_temporal=args.enable_temporal
    )
    model, input_transform, (img_w, img_h) = initialize_model(
        frames_to_process[0], args.model_weights_dir, args.enable_temporal, args.device
    )

    # Process frames
    frame_heights, frame_names, overlays, confidence_scores = process_frames(
        model, input_transform, frames_to_process, args, target_class, (img_w, img_h)
    )
    ...
    layer_metrics = analyze_layer_metrics(
        frame_heights_mm, layer_ids, confidence_scores
    )
    anomalies = detect_anomalies(frame_heights_mm, layer_ids)

    # Create and save visualization
    fig, axs = create_visualization(
        frame_heights_mm, frame_names, overlays, layer_ids, args
    )
    ...
    all_metrics = {
        "video_name": os.path.basename(
            args.image_dir
            if args.image_dir
            else args.video if args.video else args.image
        ),
