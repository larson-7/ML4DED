...
class UNetSegmentationHead(nn.Module):
    """Wraps a Conv2d segmentation head and provides extract_temporal_tokens."""
    def __init__(self, conv_head: nn.Conv2d, num_temporal_tokens: int = 1):
        super().__init__()
    ...
    def extract_temporal_tokens(self, features: torch.Tensor):
        # features: (B, C, H, W)
        # Return global average pooled features as temporal tokens
        pooled = torch.nn.functional.adaptive_avg_pool2d(features, (1, 1))  # (B, C, 1, 1)
        pooled = pooled.view(pooled.size(0), 1, -1)  # (B, 1, C)
        # Repeat if more than one token is needed
        if self.num_temporal_tokens > 1:
            pooled = pooled.repeat(1, self.num_temporal_tokens, 1)  # (B, num_tokens, C)
        return pooled