...
def main():
    ...
    for i, entry in enumerate(sorted(os.scandir(args.image_dir), key=lambda x: x.name)):
        if not entry.name.lower().endswith(('.jpg', '.png', '.jpeg')):
            continue

        if i % 10 != 0:
            continue  # Skip frames for speed (only process every 10th frame)
        ...
        scale_mm_per_px = args.expected_height_mm / height_px
        frame_names.append(entry.name)
        frame_heights.append(height_px)
        frame_scales.append(scale_mm_per_px)

        img_np = np.array(raw_img.resize((img_w, img_h)))  # Ensure size consistency
        overlay_img = overlay_segmentation(img_np, seg_map_np, alpha=0.5)
        overlays.append(overlay_img)

        print(f"{entry.name}: Height_px = {height_px}, Scale ≈ {scale_mm_per_px:.3f} mm/px")
    ...
    # Plot height vs frame (using image filenames as x-axis labels)
    axs[0].plot(range(len(frame_names)), frame_heights, marker='o')
    axs[0].set_title('CURRENT_PART Pixel Height per Frame')
    ...
    # Plot scale vs frame
    axs[1].plot(range(len(frame_names)), frame_scales, marker='o')
    axs[1].set_title('Estimated Pixel-to-mm Scale per Frame')
    axs[1].set_ylabel('mm per Pixel')
    axs[1].set_xlabel('Image Name')
    axs[1].set_xticks(range(len(frame_names)))
    # axs[1].set_xticklabels(frame_names, rotation=45, ha='right', fontsize=8)

    # Add overlay image display + slider
    fig_overlay, overlay_ax = plt.subplots(figsize=(8, 6))
    overlay_img_obj = overlay_ax.imshow(overlays[0])
    overlay_ax.axis('off')
    overlay_ax.set_title(f"Frame: {range(len(frame_names))}")
